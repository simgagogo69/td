...
const SUPPORTS = [
  {
    ...
    action: (tx, ty, support) => {
      ...
      }, Math.round(bigMarkerTimer * 1000));
    }
  },
  {
    id: 'nuke',
...
for (let run of planeRuns) {
  ...
  if (run.type === 'airstrike' || run.type === 'carpet') {
    let px = run.direction === 'lr' ? frac * canvas.width : (1-frac)*canvas.width;
    let py = run.ty;
    ctx.save();
    ctx.translate(px, py);
    const ang = run.direction === 'lr' ? 0 : Math.PI;
    ctx.rotate(ang);
    ctx.fillStyle = '#ddd';
    ctx.beginPath(); ctx.moveTo(-8,-4); ctx.lineTo(10,0); ctx.lineTo(-8,4); ctx.closePath(); ctx.fill();
    ctx.restore();
  }
}
...
function spawnICBMStrike(tx, ty, support) {
  const DAMAGE = 600; 
  const R = support.radius || 120;
  _spawnExplosionAt(tx, ty, R, DAMAGE, 46);

  for (let i = 0; i < 8; i++) {
    setTimeout(() => {
      _spawnExplosionAt(tx + (Math.random()-0.5)*40, ty + (Math.random()-0.5)*40, R * 0.55, Math.round(DAMAGE*0.25), 18);
    }, 120 + i*80);
  }

  spawnFloatingText('ICBM STRIKE', tx, ty - 36, '#ffad7f');
  drawMap(activeMap);
  updateUI();
}
...
function spawnCarpetBomb(tx, ty, orientation, support) {
  ...
  for (let i = 0; i < steps; i++) {
    const tDelay = (i / (steps-1)) * run.duration * 1000; 
    setTimeout(() => {
      let ex = tx, ey = ty;
      if (orientation === 'horizontal') {
        ex = (i / (steps-1)) * canvas.width;
        ey = ty;
      } else {
        ex = tx;
        ey = (i / (steps-1)) * canvas.height;
      }
      _spawnExplosionAt(ex, ey, support.radius || 28, 110, 18);
    }, tDelay);
  }

  const planes = 3;
  for (let p = 0; p < planes; p++) {
    const offset = (p / planes) * 0.12;
    particles.push({
      x: orientation === 'horizontal' ? -40 - p*30 : tx + (Math.random()-0.5)*20,
      y: orientation === 'horizontal' ? ty - 40 - p*6 : -40 - p*30,
      vx: orientation === 'horizontal' ? 2.8 + Math.random()*0.6 : 0,
      vy: orientation === 'horizontal' ? 0 : 2.8 + Math.random()*0.6,
      life: run.duration + 0.9,
      maxLife: run.duration + 0.9,
      size: 8,
      colorStart: [200,200,200],
      colorEnd: [180,180,180],
      plane: orientation, 
    });
  }
}
...
function spawnAirstrike(tx, ty, support) {
  planeRuns.push({ type: 'airstrike', tx, ty, start: globalTime, duration: 1.8, direction: Math.random()<0.5 ? 'lr' : 'rl' });
  spawnFloatingText('AIRSTRIKE', tx, ty-36, '#ffd76b');
}

function spawnCarpetBomb(tx, ty, orientation, support) {
  planeRuns.push({ type: 'carpet', tx, ty, orientation, start: globalTime, duration: 2.2, support });
...
function spawnEMPBlastGlobal(durationSeconds = 6) {
  globalEMP.active = true;
  globalEMP.expires = globalTime + durationSeconds;
  for (let e of enemies) {
...
function spawnNicollDysonBeam(tx, ty, duration = 6) {
  const angle = Math.atan2(ty - canvas.height/2, tx - canvas.width/2);
  const beam = {
    x: tx, y: ty,
    angle,
    start: globalTime,
    duration,
    width: 18,     
    range: Math.max(canvas.width, canvas.height) * 1.4,
    sweepSpeed: 0.7 + Math.random()*0.6, 
...
for (let i = planeRuns.length - 1; i >= 0; i--) {
  const run = planeRuns[i];
  const elapsed = globalTime - run.start;
  const frac = Math.min(1, elapsed / run.duration);
  if (run.type === 'airstrike' || run.type === 'carpet') {
    if (run.orientation === 'vertical' || run.type==='airstrike') {
      const x = run.direction === 'lr' ? frac * canvas.width : (1-frac) * canvas.width;
      const y = run.ty || run.ty === 0 ? run.ty : run.ty;
      if (Math.random() < 0.22) {
        _spawnExplosionAt(x + (Math.random()-0.5)*12, run.ty + (Math.random()-0.5)*28, run.support?.radius || 18, run.type === 'carpet' ? 120 : 80, 12);
      }
    } else { 
      const y = frac * canvas.height;
      if (Math.random() < 0.22) {
        _spawnExplosionAt(run.tx + (Math.random()-0.5)*28, y + (Math.random()-0.5)*12, run.support?.radius || 18, 120, 12);
      }
    }
  }

  if (elapsed > run.duration + 0.2) {
    planeRuns.splice(i, 1);
  }
}
...
for (let bi = beams.length - 1; bi >= 0; bi--) {
  ...
  b.angle += 0.005 * b.sweepSpeed * (Math.sin(age + b.phase));
  for (let e of enemies) {
